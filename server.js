...
app.get('/api/customers', _reqAuthCompanies, async (req, res) => {
  try {
    const { listCustomersForOwner } = require('./src/customers');
    const customers = await listCustomersForOwner(req.user.id);
    res.json(customers);
  } catch (err) {
...
app.get('/api/customers/:id', _reqAuthCompanies, async (req, res) => {
  try {
    ...
    const isPrivileged = req.user.role === 'admin';   // Phase 2: || req.user.role === 'se'
    const c = await getCustomer(req.params.id, req.user.id, isPrivileged);
