...
## v4.4.0 — Phase 1b + 1d (calculator layout)

Presentation-layer only. Engine unchanged (22/22). Additive CSS at the end of style.css plus a few class additions — revertable.

**Sticky-header fix (the important one):** the KPI live bar is position:fixed occupying the top ~116px (topbar-h + livebar-h). The v4.2.0 sticky action header was pinned at top:0, so on scroll it slid up and disappeared behind the live bar. All sticky chrome (action header, workflow steps, guided stepper) now sticks BELOW the live bar using token math (topbar-h + livebar-h + offset), with livebar-off variants for tabs/states where the KPI strip is hidden.

**1d — chrome consolidation:** the calculator stacked three competing layers (sticky action header, a floating workflow-steps card, and a benchmark banner). The action header + workflow steps now form one continuous sticky band; the workflow chips are flatter/toolbar-like; the benchmark banner sits inside the band rather than adding a third layer.

**1b — input tiering:** the five inputs that drive most of the ROI (revenue, users, labor cost, inventory value, IT cost) read as primary (larger inputs, stronger labels, more space). The fine-tuning "Industry avg" recovery-% knobs are quieted (smaller, muted fill), targeted via their shared placeholder so no markup churn.

### ⚠ Needs live verification
The sticky offsets include one height estimate (header ≈ 50px) that depends on button wrapping at a given width. On staging, check on the calculator: (1) scroll — header+workflow band stays as one unit, no overlap/gap against the KPI bar; (2) nothing hides behind the KPI bar; (3) guided mode — stepper clears the band; (4) narrow window — if the header wraps, the workflow strip still sits below it. If any offset is off, it is a one-value tweak.

Note: 11 pre-existing font-weight:800 rules reference an unloaded Inter weight — flagged, not changed.

## v4.4.1 — sticky-header revert + robust PDF fix

- **Reverted the sticky calculator header (v4.2.0 / Phase 1d).** It overlapped the customer-selection gate ("Who are you building a business case for?") and the stacked sticky positioning (header + workflow strip + guided nav, all clearing a fixed KPI bar) was too fragile. The calculator header and workflow steps scroll normally again. The Save / Executive view / Share / Email buttons remain in the header row (just not pinned). 1b input tiering is retained.
- **Executive PDF — root-caused and hardened.** The print page reads IND / COMP (industry & competitor labels), THREE_WHYS_LIBRARY and AUDIENCE_CONFIG. These are top-level `const` in their source files, and top-level `const` does NOT reliably become a window property across <script> boundaries (depends on strict-mode context) — so the print page's bare `IND[...]` reference could throw "ReferenceError", surfaced to the user as "Could not load scenario / Data may be corrupted". Fix: (1) the source files now explicitly attach these to window; (2) the print page resolves them defensively and degrades to a dash instead of throwing if any is ever missing. Verified the render succeeds, and also that it no longer crashes even with all globals absent.

## v4.4.2 — Executive PDF root-cause fix + stronger autofill block

- **Executive PDF (real root cause, verified):** the print page loads narrative.js but not app.js. narrative.js's buildExecHeadlines() calls the money/percent formatters fmtFull / fmt / fmtPct, which were defined in app.js — so it threw "fmtFull is not defined" inside render(), before reaching any of the guards added in prior attempts, and surfaced as "Could not load scenario". Previous fixes addressed a different missing symbol (IND) further down; this was upstream. Fix: extracted fmt/fmtFull/fmtPct into a shared public/format-utils.js loaded by both the calculator and the print page (before narrative.js), and attached to window. Verified by executing the full print render path headlessly — it now produces ~28KB of output instead of the error, and still degrades gracefully if data is missing.
- **Customer search autofill:** hardened attributes were not enough (Chrome ignores autocomplete=off on lone text inputs). Applied the readonly-until-focus technique — the field is readonly at load (Chrome will not autofill a readonly field) and drops readonly on focus so typing works normally. Applied to both the gate search and the calculator customer search.

## v4.5.0 — in-app AI assistant (Pass 1)

New feature. Engine unchanged (22/22). No migrations.

A floating "Ask" button (bottom-right, opposite the back-to-top button) opens a slide-up chat panel available on every screen inside the app. Answers questions about fields, calculations, how to use the application, and what the discovery questions mean.

**Grounding:** the assistant answers primarily from a knowledge base assembled from the app's own content — the ROI methodology (value levers, 15% overlap deduction, ramp phasing, payback, server-authoritative calc), field definitions, key terms (OTIF, shrinkage, carrying cost, inventory turns, WMS, MEP), the workflow, and sharing/tracking behavior. It may draw on Claude's general knowledge to add helpful context, and when it does, it explicitly flags this: "(This is general background, not specific to how this app is configured.)" It is instructed never to invent how the app's own calculations or features work.

**Security:** calls route through the existing /api/enhance server-side proxy — the API key never reaches the browser. Auth-gated and rate-limited (same as the Three-Whys enhancer). The system prompt is now forwarded by the proxy (new: the proxy previously ignored a system field). Conversation lives in memory for the session only — nothing persisted.

**Scope:** loaded in index.html only, not on prospect.html. The prospect-facing discovery page does not get the internal assistant.
