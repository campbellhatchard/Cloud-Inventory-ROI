...
async function solutionFitAccess(user,customerId,mode='view'){
  ...
  const {rows}=await query(`SELECT primary_se_id,additional_se_ids,created_by FROM handoffs WHERE customer_id=$1`,[customerId]);
